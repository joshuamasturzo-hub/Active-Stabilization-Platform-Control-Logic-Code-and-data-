...
CSV = "pd_control_data_20260403_200031.csv"
OUT = "dry test 12 PD — Roll600 D15 Pitch300 D8 filtered.png"
...
fig.suptitle("Dry Test 12 — PD | Roll P=600 D=15 | Pitch P=300 D=8 | EMA filter α=0.3 | D cap 800 ERPM",
...
ax.set_title(f"Roll | IMU2 RMS={rms_r2:.2f}°  IMU1 RMS={rms_r1:.2f}°  Reduction={red_r:.1f}%  "
             f"Freq={r2_f:.3f} Hz  Period={1/r2_f:.2f}s  Lag={roll_lag:.0f}ms", fontsize=9)
...
ax.set_title(f"Pitch | IMU2 RMS={rms_p2:.2f}°  IMU1 RMS={rms_p1:.2f}°  Reduction={red_p:.1f}%  "
             f"Last 4s: IMU1 std=0.032° — stable, no oscillation", fontsize=9)
ax.legend(fontsize=8, loc="upper right"); ax.grid(True, alpha=0.3)
...
ax.axhline(-800, color="k", lw=0.6, ls=":", alpha=0.4, label="±800 ERPM D cap")
ax.set_ylabel("D term (ERPM)", fontsize=10)
ax.set_xlabel("Time (s)", fontsize=10)
ax.set_title("D Term Contribution — well within cap, no runaway", fontsize=9)
ax.legend(fontsize=8, loc="upper right"); ax.grid(True, alpha=0.3)
