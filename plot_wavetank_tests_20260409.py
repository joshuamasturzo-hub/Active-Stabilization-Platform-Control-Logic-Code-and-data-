"""Generate individual plots for all 4 wavetank tests — exact pool test 2 format."""
...
TESTS = [
    ("fpd_tune_data_20260409_173209.csv", "Wavetank Test 1", "wavetank_test1_20260409.png"),
    ("fpd_tune_data_20260409_173617.csv", "Wavetank Test 2", "wavetank_test2_20260409.png"),
    ("fpd_tune_data_20260409_173850.csv", "Wavetank Test 3", "wavetank_test3_20260409.png"),
    ("fpd_tune_data_20260409_174117.csv", "Wavetank Test 4", "wavetank_test4_20260409.png"),
]
...
for csv_file, label, out_file in TESTS:
    ...
    fig.suptitle(
        f"{label} — FPD Control | {dur:.0f}s | Roll {red_r:.1f}% | Pitch {red_p:.1f}% | 200 Hz loop",
        fontsize=13, fontweight="bold", y=0.99,
    )

    # Panel 1 — Roll
    ax = axes[0]
    ax.plot(t, r2, "b-", alpha=0.5, lw=0.8, label="IMU2 Reference (boat)")
    ax.plot(t, r1, "r-", alpha=0.85, lw=1.0, label="IMU1 Platform (stabilized)")
    ...
    # Panel 2 — Pitch
    ax = axes[1]
    ax.plot(t, p2, "b-", alpha=0.5, lw=0.8, label="IMU2 Reference (boat)")
    ax.plot(t, p1, "r-", alpha=0.85, lw=1.0, label="IMU1 Platform (stabilized)")
    ax.axhline(0, color="k", lw=0.5, ls="--")
    ...
    ax = axes[3]
    if len(roll_per) > 0:
        ax.scatter(roll_pt, roll_per, color="b", s=30, alpha=0.8, zorder=3,
                   label=f"Roll period ({len(roll_per)} cycles)")
        ...
        title = (f"Roll Wave Period — {len(roll_per)} cycles detected | "
                 f"Median={np.median(roll_per):.2f}s | Min={roll_per.min():.2f}s | Max={roll_per.max():.2f}s")
    else:
        title = "Roll Wave Period — no cycles detected"
    ...
    ax = axes[4]
    if len(pitch_per) > 0:
        ax.scatter(pitch_pt, pitch_per, color="m", s=30, alpha=0.8, zorder=3,
                   label=f"Pitch period ({len(pitch_per)} cycles)")
        ...
        title = (f"Pitch Wave Period — {len(pitch_per)} cycles detected | "
                 f"Median={np.median(pitch_per):.2f}s | Min={pitch_per.min():.2f}s | Max={pitch_per.max():.2f}s")
    else:
        title = "Pitch Wave Period — no cycles detected"
    ax.set_ylabel("Period (s)", fontsize=10)
